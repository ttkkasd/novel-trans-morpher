...
print("names=", names)
# 请帮我写一个满足下列要求的prompt:
# 需求：将给定的城市名称（“具体的城市名称”）进行重新命名。
# 要求：
# 1.只返回新的城市名称，不能包含其他无关的回答。
# 2.新名字也应该符合一个城市名称的命名规则。
# 3.如果原来的名字中含有'城'字的话，那新的名字中也必须含有'城'字。
# 3.如果城市名称为中文，新名字也应该是中文。原来的名称和新名称的字数要相同。请反思并核对自己的回答是否有符合所有的要求。只需要回复新的城市名。
nameTemplate = """
...
"""
prompt = PromptTemplate.from_template(nameTemplate)

for name in names:
    chain = prompt | llm
    answer = chain.invoke({'name': name})
